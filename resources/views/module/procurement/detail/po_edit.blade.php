...
<form action="{{route('procurement.po.update', [$spph])}}" method="POST" enctype="multipart/form-data">
@csrf
{{ method_field('PUT') }}
<div class="modal-header">
    <h5 class="modal-title" id="exampleModalLabel">Ubah PO</h5>
</div>
                    ...
                    <div class="row">
                        <div class="col-xl-6">
                            ...
                            <div class="form-group">
                                ...
                                <input name="no_spmp" required="true" value="{{ $spph->po->no_spmp }}" class="form-control{{ $errors->has('no_spmp') ? ' is-invalid' : '' }}" type="text"/>
                        ...
                        <div class="col-xl-6">
                            <div class="form-group">
                                ...
                                <input name="date" required="true" value="{{ date('Y-m-d', strtotime($spph->po->date)) }}" class="datepicker form-control{{ $errors->has('date') ? ' is-invalid' : '' }}" placeholder="yyyy-mm-dd" type="text"/>
                                @if ($errors->has('date'))
                                    <span class="small" style="color:red" role="alert">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="approved_by" style="width:100%">
                                    @foreach($users as $user)
                                        <option @if($spph->po->approved_by == $user->id) selected @endif value="{{$user->id}}">{{$user->name}} - {{$user->jabatan_caption}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label class="small mb-1">PPN </label>
                                <input name="ppn" style="width:20px" @if($spph->po->ppn==1) checked @endif value="1" class="form-control{{ $errors->has('ppn') ? ' is-invalid' : '' }}" type="checkbox"/>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xl-12">
                            <div class="form-group">
                                ...
                                <textarea id="editor" name="ketentuan_pekerjaan" rows="4" class="form-control{{ $errors->has('ketentuan_pekerjaan') ? ' is-invalid' : '' }}">{{ $spph->po->ketentuan_pekerjaan }}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="editor2" name="ketentuan_pembayaran" rows="4" class="form-control{{ $errors->has('ketentuan_pembayaran') ? ' is-invalid' : '' }}">{{ $spph->po->ketentuan_pembayaran }}</textarea>
